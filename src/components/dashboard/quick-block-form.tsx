...
export function QuickBlockForm({ 
  ...
  return (
    <div className="space-y-8">
      {/* Block Form */}
      <form onSubmit={handleSubmit} className={`${inline ? '' : 'bg-slate-50/50 dark:bg-slate-950/30 p-6 rounded-3xl border border-slate-100 dark:border-slate-850'} space-y-4`} noValidate>
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-2xl border-2 border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-4 py-3 text-sm focus:outline-none focus:border-indigo-600 focus:bg-white dark:focus:bg-slate-950 transition-all dark:text-white hover:border-indigo-200 dark:hover:border-slate-700"
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full rounded-2xl border-2 bg-slate-50 dark:bg-slate-950 px-4 py-3 text-xs focus:outline-none focus:bg-white dark:focus:bg-slate-950 transition-all dark:text-white ${
                    fieldErrors.startTime ? "border-rose-200 bg-rose-50/30 focus:border-rose-500" : "border-slate-200 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-slate-700 focus:border-indigo-600"
                  }`}
                />
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full rounded-2xl border-2 bg-slate-50 dark:bg-slate-950 px-4 py-3 text-xs focus:outline-none focus:bg-white dark:focus:bg-slate-950 transition-all dark:text-white ${
                    fieldErrors.endTime ? "border-rose-200 bg-rose-50/30 focus:border-rose-500" : "border-slate-200 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-slate-700 focus:border-indigo-600"
                  }`}
                />
      ...
      {!inline && (
        <div className="space-y-4">
            ...
            <div className="text-center py-10 bg-slate-50/30 dark:bg-slate-950/20 rounded-3xl border-2 border-dashed border-slate-100 dark:border-slate-800/60">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter">No active blocks for today or future.</p>
            </div>
            ) : (
            <div className="grid gap-3">
                {existingBlocks.map((block) => (
                <div key={block.id} className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-950/45 border border-slate-100 dark:border-slate-800/80 rounded-2xl group hover:border-indigo-200 dark:hover:border-slate-700 transition-all shadow-sm">
