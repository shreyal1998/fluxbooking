...
export function ManualBooking({ 
  ...
  // Auto-advance if data provided via initialData
  useEffect(() => {
    if (initialData?.startTime && initialData?.staffId && selectedService) {
        setStep(3);
    }
  }, []);
  ...
  const content = (
    <div className={`relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-[2.5rem] ${inline ? '' : 'shadow-2xl border border-slate-100 dark:border-slate-700'} overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh] transition-colors`}>
        ...
        <div className={`flex-1 overflow-y-auto ${inline ? 'p-0' : 'p-8'}`}>
            {step === 1 && (
            ...
            <div className="space-y-8">
                ...
                <div className="flex gap-3">
                    <button 
                        ...
                        className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 dark:shadow-none border border-transparent dark:border-white/10 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50"
                    >
                        {loading ? "Saving..." : (
                        <>
