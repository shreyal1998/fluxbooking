...
export function LeaveRequestsManager({ initialRequests, timeFormat = "12h" }: { initialRequests: any[], timeFormat?: string }) {
  ...
  if (requests.length === 0) {
    return (
      <div className="bg-slate-50 dark:bg-slate-900/50 border border-dashed border-indigo-100/80 dark:border-slate-800 p-12 rounded-[2rem] text-center transition-colors">
  ...
  return (
    <div className="flex flex-col gap-4 transition-colors">
      {requests.map((request) => (
        <div key={request.id} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-indigo-100/80 dark:border-slate-800 shadow-soft group hover:shadow-xl hover:shadow-indigo-500/5 transition-all">
          ...
          <div className="space-y-3 pt-4 border-t border-indigo-100/50 dark:border-slate-800">
            ...
            <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400">
              ...
              <p className="text-xs font-bold">
                {format(new Date(request.startTime), timeDisplayFormat)} - {format(new Date(request.endTime), timeDisplayFormat)}
              </p>
            </div>
            {request.reason && (
