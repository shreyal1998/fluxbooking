...
export function BookingsClient({ 
  ...
  const currentStaffId = useMemo(() => staff.find((s: any) => s.userId === userId)?.id, [staff, userId]);
  const canManage = useCallback((staffId?: string) => {
    if (userRole === "ADMIN") return true;
    return Boolean(staffId && currentStaffId && staffId === currentStaffId);
  ...
  const allowedStaffForBooking = useMemo(() => {
    if (userRole === "STAFF" && currentStaffId) {
      return staff.filter((s: any) => s.id === currentStaffId);
    }
    return staff;
  }, [userRole, currentStaffId, staff]);

  const handleSlotClick = (date: Date, staffId?: string) => {
    if (userRole === "STAFF" && staffId && currentStaffId && staffId !== currentStaffId) {
      toast.error("Staff members can only create bookings for their own schedule.");
      return;
    }
    setSelectedSlotInfo({ date, staffId: userRole === "STAFF" ? (currentStaffId || staffId) : staffId });
  ...
  const listFilteredBookings = useMemo(() => {
    if (currentStaffFilter.includes("all") || currentStaffFilter.length === 0) return initialBookings;
    if (currentStaffFilter.includes("none")) return [];
    return initialBookings.filter((b) => currentStaffFilter.includes(b.staffId));
  }, [initialBookings, currentStaffFilter]);
  ...
  const filteredEvents = useMemo(() => {
    ...
    const isFiltered = !currentStaffFilter.includes("all") && currentStaffFilter.length > 0;

    if (isFiltered) {
      filteredBookings = currentStaffFilter.includes("none") ? [] : filteredBookings.filter((b) => currentStaffFilter.includes(b.staffId));
      filteredBlocked = currentStaffFilter.includes("none") ? [] : filteredBlocked.filter((s) => currentStaffFilter.includes(s.staffId));
      filteredOverrides = currentStaffFilter.includes("none") ? [] : filteredOverrides.filter((o) => currentStaffFilter.includes(o.staffId));
      filteredLeaves = currentStaffFilter.includes("none") ? [] : filteredLeaves.filter((l: any) => currentStaffFilter.includes(l.staffId));
    }
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-right">
                              <div className="flex items-center justify-end gap-1 transition-all">
                                {canManage(booking.staffId) ? (
                                  ...
                                  <span className="text-[11px] text-slate-400 dark:text-slate-500 font-medium italic pr-2">Read-only</span>
                                )}
                              </div>
                            </td>
                          </tr>
