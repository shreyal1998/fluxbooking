    ...
    <div className="flex-1 flex flex-col animate-fade-in p-4 md:p-6 lg:p-8">
      ...
      <div className="flex-1 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 sm:px-8 sm:py-4.5 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg sm:text-xl font-medium text-slate-900 dark:text-slate-200 tracking-tight">{labels.customer}s</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            {userRole === "ADMIN" && (
              <div className="hidden sm:flex bg-slate-50 dark:bg-slate-900 p-0.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex-1 p-6 sm:p-8 pt-6" id="customers-table">
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                  <thead>
                    <tr className="bg-indigo-50/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
                      <th className="px-6 py-4 sm:px-8 sm:py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{labels.customer}</th>
                      <th className="px-6 py-4 sm:px-8 sm:py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Email</th>
                      <th className="px-6 py-4 sm:px-8 sm:py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Phone Number</th>
                      {userRole === "ADMIN" && (
                        <th className="px-6 py-4 sm:px-8 sm:py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th>
                      )}
                      ...
                      <tr key={customer.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 group">
                        <td className="px-6 py-4 sm:px-8 sm:py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className={`text-sm font-medium transition-colors ${customer.status === 'ACTIVE' ? 'text-slate-950 dark:text-slate-50' : 'text-slate-400'}`}>{customer.name}</p>
                              {customer.notes && (
                        ...
                        <td className="px-6 py-4 sm:px-8 sm:py-4 whitespace-nowrap">
                          {customer.phone ? (
                            ...
                            <div className="flex items-center justify-center w-28 text-xs text-slate-400 font-medium">
                              —
                            </div>
                          )}
                        </td>
                            ...
                            {customer.status === 'ACTIVE' ? (
                              <Tooltip content="Inactivate" position="bottom">
                                <button 
                                  ...
                                  className="p-2 rounded-lg bg-transparent text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-800 transition-all border border-transparent active:scale-95 disabled:opacity-50 cursor-pointer"
                                >
               ...
               <div className="px-8 py-6 border-b border-indigo-100/50 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white dark:bg-slate-900 rounded-t-[2.4rem] z-10">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        ...
                        <p className="text-xs font-medium text-slate-500">New {labels.customer.toLowerCase()} file</p>
                     </div>
                  </div>
                  <button 
                     onClick={() => setIsAddModalOpen(false)} 
               ...
               <div className="px-8 py-6 border-b border-indigo-100/50 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white dark:bg-slate-900 rounded-t-[2.4rem] z-10">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        ...
                        <p className="text-xs font-medium text-slate-500">Configuring {editingCustomer.name}</p>
                     </div>
                  </div>
                  <button 
                     onClick={() => setEditingCustomer(null)} 
                  ...
                  <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6 premium-scrollbar">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Enter customer notes, preferences, or important history..."
                        className="w-full rounded-2xl border-2 border-indigo-100/50 dark:border-slate-800 bg-indigo-50/30 dark:bg-slate-900 px-5 py-3 text-sm focus:outline-none transition-all dark:text-white shadow-sm hover:border-indigo-200 dark:hover:border-slate-800 focus:border-indigo-600 focus:bg-white dark:focus:bg-slate-900"
                      />
                    </div>
                  </div>
