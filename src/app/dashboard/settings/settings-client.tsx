      ...
      <div className="flex-shrink-0 mb-10 px-4">
        <div className="flex flex-wrap items-center bg-slate-50 dark:bg-slate-800 p-1.5 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm self-start inline-flex">
          {tabs.map((tab) => (
            ...
            >
              <tab.icon className={`h-4 w-4 ${activeTab === tab.id ? "text-indigo-600 dark:text-indigo-400" : "opacity-60"}`} />
              <div className="text-left">
                <div className="text-[10px] font-medium uppercase tracking-widest leading-none mb-0.5">{tab.label}</div>
                <div className={`text-[9px] font-medium whitespace-nowrap hidden sm:block ${activeTab === tab.id ? "text-indigo-400 dark:text-indigo-300/60" : "text-slate-900 dark:text-white opacity-40"}`}>
                  {tab.description}
                </div>
              </div>
            </button>
